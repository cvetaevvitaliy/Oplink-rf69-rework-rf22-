...
bool RFM69_initialize(uint8_t freqBand, uint8_t nodeID, uint8_t networkID)
{
  const uint8_t CONFIG[][2] =
  {
		...
		// /* 0x01 */ { REG_OPMODE, RF_OPMODE_SEQUENCER_ON | RF_OPMODE_LISTEN_ON | RF_OPMODE_STANDBY },
...
void RFM69_interruptHandler() {
  ...
  if ((_mode == RF69_MODE_RX || _mode == RF69_MODE_LISTEN) && (RFM69_readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY))
  {
    uint8_t CTLbyte;
...
//ATOMIC_BLOCK(ATOMIC_FORCEON)
  ...
  if ((_mode == RF69_MODE_RX || _mode == RF69_MODE_LISTEN) && payloadLen > 0)
  {
...
void RFM69_listen(bool onOff)
{
	if (onOff==true)
		{
			RFM69_writeReg(REG_OPMODE, RF_OPMODE_SEQUENCER_ON | RF_OPMODE_STANDBY);
			//RFM69_writeReg(REG_OPMODE, RF_OPMODE_LISTEN_ON | RF_OPMODE_STANDBY);
			RFM69_writeReg(REG_OPMODE, RF_OPMODE_SEQUENCER_ON| RF_OPMODE_LISTEN_ON | RF_OPMODE_STANDBY);
			_mode = RF69_MODE_LISTEN;
		}
		else
		{
			RFM69_writeReg(REG_OPMODE, RF_OPMODE_LISTEN_OFF | RF_OPMODE_LISTENABORT | RF_OPMODE_STANDBY);
			RFM69_writeReg(REG_OPMODE, RF_OPMODE_LISTEN_OFF | RF_OPMODE_STANDBY);
			_mode = RF69_MODE_STANDBY;
		}

}

void RFM69_initialize_listen()
{
	/*RFM69_writeReg(REG_LISTEN1, 0x94);
	RFM69_writeReg(REG_LISTEN2, 1); //0x40: 64*4.1ms~262ms idle
	RFM69_writeReg(REG_LISTEN3, 255); //0x20: 32*64us~2ms RX
    //RFM69_writeReg(REG_LISTEN2, 0x10); //0x40: 64*4.1ms~262ms idle
    //RFM69_writeReg(REG_LISTEN3, 0x80); //0x20: 32*64us~2ms RX
    RFM69_writeReg(REG_RXTIMEOUT2, 0x1F);//needed otherwise will always be in RX mode*/
  RFM69_writeReg(REG_LISTEN1, 0x92);
